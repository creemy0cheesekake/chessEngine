...
CUSTOM_TEST_CASE("Test Pawn Moves") {
	SUBCASE("Test Pawn Promotion") {
		...
		b.execute(m);
		CHECK(m.getFlags() == PAWN_MOVE + PROMOTION);
		CHECK(b.boardState.hmClock == 0);
		CHECK(b.boardState.fmClock == 9);
	...
	SUBCASE("Test Double Pawn Push") {
		...
		b.execute(m);
		CHECK(m.getFlags() == DBL_PAWN + PAWN_MOVE);
		CHECK(b.boardState.hmClock == 0);
		CHECK(b.boardState.fmClock == 1);
	...
	SUBCASE("Test Pawn Capture") {
		...
		b.execute(m);
		CHECK(m.getFlags() == CAPTURE + PAWN_MOVE);
		CHECK(b.boardState.hmClock == 0);
		CHECK(b.boardState.fmClock == 2);
		CHECK(b.boardState.sideToMove == BLACK);
	}
	SUBCASE("Test En Passant") {
		...
		CHECK(b.boardState.enPassantSquare == 1UL << f6);
		b.execute(m);
		CHECK(m.getFlags() == CAPTURE + EN_PASSANT + PAWN_MOVE);
		CHECK(b.boardState.hmClock == 0);
		CHECK(b.boardState.fmClock == 3);
		CHECK(b.boardState.sideToMove == BLACK);
	}
}
...
CUSTOM_TEST_CASE("Test Castling") {
	SUBCASE("White Kingside Castling") {
		...
		b.execute(m);
		CHECK(m.getFlags() == KS_CASTLE);
		CHECK(b.boardState.hmClock == 5);
		CHECK(b.boardState.fmClock == 4);
		CHECK(b.boardState.sideToMove == BLACK);
		CHECK((bool)(b.boardState.castlingRights.rights & ~0b1100));
	}
	SUBCASE("White Queenside Castling") {
		...
		b.execute(m);
		CHECK(m.getFlags() == QS_CASTLE);
		CHECK(b.boardState.hmClock == 7);
		CHECK(b.boardState.fmClock == 6);
		CHECK(b.boardState.sideToMove == BLACK);
		CHECK((bool)(~b.boardState.castlingRights.rights & ~0b1100));
	}
	SUBCASE("Black Kingside Castling") {
		...
		b.execute(m);
		CHECK(m.getFlags() == KS_CASTLE);
		CHECK(b.boardState.hmClock == 6);
		CHECK(b.boardState.fmClock == 6);
		CHECK(b.boardState.sideToMove == WHITE);
		CHECK((bool)(b.boardState.castlingRights.rights & ~0b0011));
	}
...
CUSTOM_TEST_CASE("Test Rook Affecting Castling") {
	SUBCASE("White Kingside Castling") {
		...
		b.execute(m);
		CHECK(m.getFlags() == NORMAL_MOVE);
		CHECK(b.boardState.hmClock == 3);
		CHECK(b.boardState.fmClock == 2);
		CHECK(b.boardState.sideToMove == BLACK);
		CHECK((bool)(~b.boardState.castlingRights.rights & ~0b1));
	}
	SUBCASE("White Queenside Castling") {
		Board b = Board();
		...
		b.execute(m);
		CHECK(m.getFlags() == NORMAL_MOVE);
		CHECK(b.boardState.hmClock == 1);
		CHECK(b.boardState.fmClock == 2);
		CHECK(b.boardState.sideToMove == BLACK);
		CHECK((bool)(~b.boardState.castlingRights.rights & ~0b01));
	}
	SUBCASE("Black Kingside Castling") {
		Board b = Board();
		...
		b.execute(m);
		CHECK(m.getFlags() == NORMAL_MOVE);
		CHECK(b.boardState.hmClock == 2);
		CHECK(b.boardState.fmClock == 3);
		CHECK(b.boardState.sideToMove == WHITE);
		CHECK((bool)(~b.boardState.castlingRights.rights & ~0b001));
	}
	SUBCASE("Black Queenside Castling") {
		Board b = Board();
		...
		CHECK(b.boardState.fmClock == 3);
		CHECK(b.boardState.sideToMove == WHITE);
		CHECK((bool)(~b.boardState.castlingRights.rights & ~0b0001));
	}
}
...
CUSTOM_TEST_CASE("Test King Affecting Castling") {
	SUBCASE("White Castling") {
		...
		CHECK(b.boardState.fmClock == 2);
		CHECK(b.boardState.sideToMove == BLACK);
		CHECK((bool)(~b.boardState.castlingRights.rights & ~0b11));
	}
