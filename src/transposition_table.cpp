...
const TTEntry& TranspositionTable::getEntry(ZobristHash h) {
	size_t index = h % m_size;
...
void TranspositionTable::printCapacity() {
	...
	std::cout << "Transposition Table Capacity: " << m_used << " / " << m_size << " = " << m_used / (float)m_size * 100.0f << " %\n";
}
